...
const isSuccess = (res) => res?.code === 1 || String(res?.status).toUpperCase() === 'SUCCESS'

export const cardService = {
  getList: async ({ page = 1, num_data = 20, card_status, pin_status, customer_id } = {}) => {
    ...
    if (!isSuccess(res)) throw new Error(res?.message || 'Failed to fetch card list')
    return { data: res?.data ?? [], message: res?.message }
  },

  getCard: async (card_id) => {
    const res = await callApi(CARD_FETCH, { card_id: Number(card_id) })
    if (!isSuccess(res)) throw new Error(res?.message || 'Failed to fetch card details')

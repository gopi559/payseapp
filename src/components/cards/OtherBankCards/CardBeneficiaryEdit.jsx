...
const CardBeneficiaryEdit = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!id) {
      toast.error(t('invalid_beneficiary'))
      return
    }
    if (!validate()) {
        ...
        <div className="w-full max-w-2xl">
          <div className="flex items-center justify-between gap-3 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold" style={{ color: contentCard.title }}>{t('edit_card_beneficiary')}</h2>
                <p className="text-sm" style={{ color: contentCard.subtitle }}>
                  {row?.masked_card ? `${t('card')}: ${row.masked_card}` : t('update_beneficiary_details')}
                </p>
              </div>
            </div>
